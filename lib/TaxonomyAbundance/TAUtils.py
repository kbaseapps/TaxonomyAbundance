...
class GraphData:
    ...
    def graph_by_group(self, level, legend_font_size, cutoff, category_field_name):
        ...
        for x, y in self.the_dict.items():  # x = taxonomic_string, y = cumulative_percentage
            ...
            i += 1
        plt.xticks(range(-1, len(x_tick_grp_str_l)), x_tick_grp_str_l, rotation=90)
    ...
    def graph_all(self, level, legend_font_size, cutoff):
        ...
        i = 0
        for x, y in self.the_dict.items():
            plt.bar(bars, y, label=(x + ('('+str(self.other_count)+') cutoff: '+str(cutoff) if x == 'Other' else '')), color=("#%06x" % random.randint(0, 0xFFFFFF)))
            i += 1
        plt.title('Level: ' + taxonomy_levels[level-1])
        plt.xlabel("Samples")
        plt.xticks(bars, self.samples, rotation=90)
...
def run(amp_id, row_attributes_id, attri_map_id, grouping_label, threshold, taxonomic_level, callback_url, token, scratch):
    ...
    try:
        ...
        grouping_label = ""
    g1.graph_this(level=int(taxonomic_level), legend_font_size=8, cutoff=threshold, peek='all', category_field_name=grouping_label)
    return {
